...
import { Edit3, Folder, Lightbulb, Wand2, Sparkles, Heart } from 'lucide-react';
...
const Sidebar = ({ activeTab, setActiveTab, isOpen, onClose }) => {
  ...
  return (
    <aside className={cn(
      "fixed md:sticky md:top-0 w-[280px] bg-navy-900/90 backdrop-blur-3xl flex flex-col h-full md:h-screen text-slate-300 border-r border-white/5 z-[50] transition-transform duration-500 ease-in-out md:translate-x-0",
      isOpen ? "translate-x-0 shadow-[20px_0_60px_rgba(0,0,0,0.4)]" : "-translate-x-full"
    )}>
      {/* Mobile Close Button */}
      <button 
        onClick={onClose}
        className="md:hidden absolute top-6 right-6 p-2.5 rounded-xl bg-white/5 text-slate-400 hover:text-white transition-all active:scale-90"
      >
      ...
      <nav className="flex-1 px-5 py-4 overflow-y-auto custom-scrollbar flex flex-col gap-8">
        <div>
          ...
          <div className="space-y-1.5">
            <SidebarItem icon={Edit3} label="Generate Post" active={activeTab === 'Dashboard'} onClick={() => setActiveTab('Dashboard')} />
            <SidebarItem icon={Wand2} label="Post Enhancer" active={activeTab === 'Post Enhancer'} onClick={() => setActiveTab('Post Enhancer')} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            <SidebarItem icon={Folder} label="My Posts" active={activeTab === 'My Posts'} onClick={() => setActiveTab('My Posts')} />
            <SidebarItem icon={Lightbulb} label="AI Ideas" active={activeTab === 'AI Ideas'} onClick={() => setActiveTab('AI Ideas')} />
          </div>
        </div>
      </nav>
